...
The environment is designed to train an agent to maintain the rocket close to the origin of the coordinate system by adjusting the nozzle orientation, $\theta$, which is limited to a maximum angle of $\frac{\pi}{6}$. The system dynamics are simulated by numerically solving the equations of motion using a fixed-step fourth-order Runge–Kutta (RK4) method. The generalized coordinates chosen are the position of the center of mass, $\vec{r}_{CM} = (x, y)$, and the rotation of the rocket about its center of mass, $\phi$.
